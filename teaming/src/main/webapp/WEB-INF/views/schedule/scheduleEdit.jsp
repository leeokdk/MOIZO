...
<body>
<div class="container">
<form:form action="scheduleModify" method="post" modelAttribute="moimSchedule" id="id_save">
				...
				<tr>
					<td colspan="2">
						...
						<div class="pull-right">
							<input type="button" value="삭제" onClick="fn_delete()" class="btn btn-sm btn-danger">	
...
function fn_save1() {
		Swal.fire({
			  ...
			  if (result.isConfirmed) {
					...
					$.ajax({
						 type		: 'POST'
						,url 		: '<c:url value="/schedule/scheduleModify" />'
...
function fn_delete() {
		Swal.fire({
			  ...
			  if (result.isConfirmed) {
				// form액션 속성을 변경 해줌
				$("#id_save").attr("action", "scheduleDelete");
				
				$.ajax({
					 type		: 'POST'
					,url 		: '<c:url value="/schedule/scheduleDelete"/>'
					,data 		: {"moimCode":$("#moimCode").val(),"mScheduleNo":$("#mScheduleNo").val(),"mScheduleModDate":$("#mScheduleModDate").val(),
								   "mScheduleStartTime":$("#mScheduleStartTime").val(),"mScheduleEndTime":$("#mScheduleEndTime").val(),
								   "mScheduleEnterMax":$("#mScheduleEnterMax").val(),"mSchedulePlace":$("#mSchedulePlace").val()} 
					,success	: function (data) {
