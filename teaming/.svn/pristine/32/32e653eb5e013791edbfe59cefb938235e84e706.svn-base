...
<style type="text/css">
body,html,list-group-item {font-family: 'Gowun Batang', serif;}
</style>
...
<body>
	...
	<c:if test="${aNotice ne null }">
		...
		<div class="container">
			...
			<!-- // START : 댓글 목록 영역  -->
			<div>
				<ul class="list-group-flush"  id="id_reply_list_area"> 
				<!-- 스크립트부분 -->
				</ul>
			</div>

			<div class="row text-center" id="id_reply_list_more">
				<a id="btn_reply_list_more" class="btn btn-sm btn-default"> 
			...
			<div class="panel panel-default">
				<div class="panel-body form-horizontal">
					<form name="frm_reply"
						...
						<input type="hidden" name="reCode" value="NTAD">
						<div class="form-group" style="margin-top: 4%;">
							<label class="col-sm-2  control-label" style="margin-bottom: 1%;"><strong>댓글</strong></label>
								<textarea rows="3" name="reContent" class="form-control" style="resize: none; width: 93%; display: inline;"><c:if test="${sessionScope.PRECONT ne null}">${sessionScope.PRECONT }</c:if></textarea>
								<button id="btn_reply_regist" type="button" class="btn btn-sm btn-outline-info" style="margin-bottom: 2%; margin-left: 1%;">등록</button>
					...
					<div class="modal-content">
						<form name="frm_reply_edit"
							...
							<div class="modal-footer">
								...
								<button id="id_exit" type="button" class="btn btn-default btn-sm" onclick="id_exit()"
...
<script type="text/javascript">
	...
	//세가지 한번에
	var params={"curPage":1, "rowSizePerPage":10, "reCode":"${aNotice.boardType}", "reParentNo":${aNotice.aNoticeNo}};
	
	// 댓글목록을 구하는 함수
	function fn_reply_list() {	
		console.log('fn_reply_list함수 실행');
			 	...
			 	if(data.result){ //true, false 반환
			 		...
			 		//해당구간을 반복
						...
						str+=' <li class="list-group-item">'
						    +' <div class="row" data-re-no='+ elt.reNo +'>	' //data~부분은 수정,삭제할때 사용
							+'		<div class="col-sm-2 text-right">'+ elt.reMemName +'</div> '
							+'		<div class="col-sm-6">	'	+ elt.reContent  +'	</div>	'
							+'		<div class="col-sm-2">'+ elt.reRegDate +'</div>	' //mod가 null이 아니면 mod
							+'		<div class="col-sm-2">	';
						if('${userInfoMap.get('NOPE').getUserId()}'==elt.reMemId){
